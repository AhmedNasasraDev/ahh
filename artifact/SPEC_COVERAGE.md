...
| F27 | layout — production | תוקן (באישור). שדה הכתיבה וכפתור «שליחה» של הצ׳אט היו מתחת לקו הקיפול בטלפון, כי רשימת ההודעות הוגבלה ב-58dvh — שבר מגובה החלון, שאינו יודע כמה מקום כבר לקחו כותרת הקבוצה והטאבים. עכשיו טאב הצ׳אט הוא עמודה שממלאת את המסגרת בדיוק (.pageChat), רשימת ההודעות לוקחת את מה שנשאר והיא היחידה שנגללת, וה-composer לא מצטמצם. נמדד ב-402×874: העמוד 815 בדיוק כמו אזור התוכן, הרשימה 321px, ה-composer 695–815 — כולו על המסך. בדיקות רגרסיה: GroupScreen.test.tsx (איזה layout כל טאב מבקש — אדומה בלי התיקון) ו-responsive.mjs (הכפתור בתוך החלון ואפס גלילה סביבו, בחמישה רוחבים). | `apps/web/src/features/groups/GroupChat.module.css (.scroll max-height: 58dvh) · routes/GroupScreen.module.css` | תוקן |
