...
- התצוגה מריצה את המסכים של המוצר ללא שינוי, עם MemoryRouter במקום BrowserRouter — לעמוד Artifact אין שליטה בשורת הכתובת.
- capabilities() בתצוגה מדווח source: 'supabase', canWrite: true. זה שקר לגבי התצוגה, והדרך היחידה לראות איך המסכים נראים לחשבון מחובר. שום בקשה אינה יוצאת מהעמוד.
- כל הנתונים הם ARTIFACT FIXTURE חוץ מ-5 מתכוני הדמו (data/demoRecipes.ts — נתוני המוצר עצמו).
- אין Supabase, אין auth, אין Realtime, אין Storage ואין Edge Function. פעולה שנראית כאילו הצליחה — הצליחה בזיכרון הדפדפן בלבד.
- URL חתום לתמונה מחזיר null בתצוגה — זהו מצב אמיתי שהגלריה מטפלת בו.
- /__inspector/auth אינו route של המוצר. AuthScreen מוצג בפרודקשן דרך AuthGate כשאין session.
